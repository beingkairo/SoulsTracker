...
$releaseBodyPath = Join-Path $root "docs\releases\v1.2.0.md"
...
$requirements = @(
    ...
    @{ Path = $releaseBodyPath; Content = $releaseBody; Text = "SoulsTracker v1.2.0" },
    @{ Path = $releaseBodyPath; Content = $releaseBody; Text = "SoulsTracker is read-only" },
    @{ Path = $releaseWorkflowPath; Content = $releaseWorkflow; Text = "body_path: docs/releases/v1.2.0.md" },
