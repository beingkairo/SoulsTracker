...
public sealed class SqliteTrackerStateRepository : ITrackerStateRepository
{
    ...
    private static PersistentTrackerState ToDomain(StoredState dto, string? token, bool applyLegacyBossListTitleMigration)
    {
        ...
        try { eldenRingSave = new EldenRingSaveConfiguration(dto.EldenRingSavePath, dto.EldenRingSaveSlotIndex ?? 0, dto.EldenRingBossListScope is int scope && Enum.IsDefined((EldenRingBossListScope)scope) ? (EldenRingBossListScope)scope : EldenRingBossListScope.AllBosses, dto.EldenRingRequiredBossesOnly ?? false); }
