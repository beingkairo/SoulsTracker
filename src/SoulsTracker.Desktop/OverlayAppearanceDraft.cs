...
public sealed class OverlayAppearanceDraft : INotifyPropertyChanged
{
    ...
    private string lastEnabledBackgroundOpacity = "88";
    ...
    public string BackgroundOpacity
    {
        ...
        set
        {
            Set(ref backgroundOpacity, value);
            if (int.TryParse(value, out int opacity) && opacity > 0)
            {
                lastEnabledBackgroundOpacity = value;
            }
        }
    }
    /// <summary>Uses the existing zero-opacity background representation when disabled.</summary>
    public bool BackgroundEnabled
    {
        get => backgroundEnabled;
        set
        {
            if (EqualityComparer<bool>.Default.Equals(backgroundEnabled, value)) return;
            Set(ref backgroundEnabled, value);
            if (value && string.Equals(BackgroundOpacity, "0", StringComparison.Ordinal)) BackgroundOpacity = lastEnabledBackgroundOpacity;
        }
    }
    ...
    public OverlayAppearance ToDomain(OverlayTextAlignment forcedAlignment)
    {
        ...
        try
        {
            ...
            return new(Title, FontFamily, Parse(FontSize, "fontSize"), TextColor, AccentColor, BackgroundColor, opacity, Parse(Padding, "padding"), Parse(CornerRadius, "cornerRadius"), forcedAlignment, OutlineEnabled, OutlineColor, Parse(OutlineWidth, "outlineWidth"), ShadowEnabled, ShadowColor, Parse(ShadowOffsetX, "shadowOffsetX"), Parse(ShadowOffsetY, "shadowOffsetY"), Parse(ShadowBlur, "shadowBlur"), Parse(TextOpacity, "textOpacity"), IconColor);
        }
