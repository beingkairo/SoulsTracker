...
public sealed class SerializedTrackerCoordinator : IAsyncDisposable
{
    ...
    private async Task ProcessAsync()
    {
        await foreach (CoordinatorRequest request in requests.Reader.ReadAllAsync().ConfigureAwait(false))
        {
            ...
            if (request is HotkeyRequest hotkeyRequest)
            {
                try
                {
                    ...
                    PersistentTrackerState updated = new(
                        ...
                        current.DeathSound,
                        current.TextExports, current.ManualDemonsSoulsDeathCounter, current.EldenRingNoticeAcknowledged, current.EldenRingSave, current.BossListScope, current.ManualBlackMythWukongDeathCounter, current.BlackMythWukongSave);
            ...
            {
                try { PersistentTrackerState current = committedState!; PersistentTrackerState updated = new(current.SchemaVersion, current.SelectedGameId, current.ManualBloodborneDeathCounter, current.BossProgress, current.OverlayConfiguration, current.ManualBloodborneHotkeys, current.DeathSound, current.TextExports, current.ManualDemonsSoulsDeathCounter, current.EldenRingNoticeAcknowledged, current.EldenRingSave, current.BossListScope, current.ManualBlackMythWukongDeathCounter, blackMythWukongSaveRequest.Configuration); await repository.SaveAsync(updated, blackMythWukongSaveRequest.CancellationToken).ConfigureAwait(false); committedState = updated; await publisher.PublishAsync(new TrackerStateChanged(updated, TrackerCommandType.UpdateBlackMythWukongSaveConfiguration), blackMythWukongSaveRequest.CancellationToken).ConfigureAwait(false); blackMythWukongSaveRequest.Completion.TrySetResult(updated); }
